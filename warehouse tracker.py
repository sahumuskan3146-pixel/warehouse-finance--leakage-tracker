...
currency = st.sidebar.radio("Currency Switcher", ["INR (₹)", "USD ($)"])
rate = 83.5 if currency == "USD ($)" else 1.0

# Dynamic Slicers
sector_slice = st.sidebar.multiselect("Filter by Sector", sales['Delivery_Sector'].unique(), default=sales['Delivery_Sector'].unique())
date_slice = st.sidebar.date_input("Date Range", [pd.to_datetime('2026-05-27'), pd.to_datetime('2026-06-02')])
...
with tab2:
    ...
    @media print {
        ...
        .stMetric { background-color: #1e1e1e !important; color: white !important; }
        .css-1544g2n { display: none !important; } /* Sidebar hide for print */
    }
    </style>
""", unsafe_allow_html=True)
    if st.sidebar.button("🖨️ Open Print/PDF Dialog"):
...
def create_download_link(val, filename):
    ...
    return f'<a href="data:application/octet-stream;base64,{b64}" download="{filename}.pdf">Click Here to Download Report</a>'
...
if st.sidebar.button("💾 Generate & Download PDF"):
    ...
    # Yahan hum aapka data add kar sakte hain
    pdf.cell(200, 10, txt=f"Total Revenue: {total_revenue}", ln=True)
    
    html = create_download_link(pdf.output(dest='S').encode('latin-1'), "FinOps_Report")
